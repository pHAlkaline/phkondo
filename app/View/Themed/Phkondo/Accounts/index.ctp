                    ...
                    <tbody>
                        <?php foreach ($accounts as $index => $account): ?>
                            <tr <?= $index == 0 ? 'data-expanded="true"' : ''; ?> >
                                ...
                                <td class="amount"><?php echo number_format($account['Account']['balance'], 2); ?>&nbsp;<?php echo Configure::read('Application.currencySign'); ?></td>
                                <td class="actions hidden-print">
                                    ...
                                    <?php echo $this->Form->postLink('<span class="glyphicon glyphicon-remove"></span> ', array('action' => 'delete', $account['Account']['id'], '?' => $this->request->query), array('title' => __('Remove'), 'class' => 'btn btn-default btn-xs', 'escape' => false, 'confirm' => __('DANGER!!! ALL ASSOCIATED DATA WILL BE LOST!').PHP_EOL.__('Are you sure you want to delete # %s?', $account['Account']['title']))); ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
