                ...
                <table data-empty="<?= __('Empty'); ?>"  class="footable table table-hover table-condensed">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th data-breakpoints="xs"><?php echo $this->Paginator->sort('payment_due_date'); ?></th>
                            <th data-breakpoints="xs"><?php echo $this->Paginator->sort('InvoiceConferenceStatus.name', __('Invoice Conference Status')); ?></th>
                    ...
                    <tbody>
                        <?php foreach ($invoices as $invoice_conference): ?>
                            <tr>
                                ...
                                <td class="amount"><?php echo number_format($invoice_conference['InvoiceConference']['amount'], 2); ?>&nbsp;<?php echo Configure::read('Application.currencySign'); ?></td>
                                <td class="actions hidden-print">
                                    <?php echo $this->Html->link('<span class="glyphicon glyphicon-list"></span> ', array('action' => 'view', $invoice_conference['InvoiceConference']['id'], '?' => ['condo_id'=>$invoice_conference['InvoiceConference']['condo_id']]), array('title' => __('Details'), 'class' => 'btn btn-default btn-xs', 'escape' => false)); ?>
                                    <?php echo $this->Html->link('<span class="glyphicon glyphicon-edit"></span> ', array('action' => 'edit', $invoice_conference['InvoiceConference']['id'], '?' => ['condo_id'=>$invoice_conference['InvoiceConference']['condo_id']]), array('title' => __('Edit'), 'class' => 'btn btn-default btn-xs', 'escape' => false)); ?>
                                    <?php echo $this->Form->postLink('<span class="glyphicon glyphicon-remove"></span> ', array('action' => 'delete', $invoice_conference['InvoiceConference']['id'], '?' => ['condo_id'=>$invoice_conference['InvoiceConference']['condo_id']]), array('title' => __('Remove'), 'class' => 'btn btn-default btn-xs', 'escape' => false, 'confirm' => __('DANGER!!! ALL ASSOCIATED DATA WILL BE LOST!').PHP_EOL.__('Are you sure you want to delete # %s?', $invoice_conference['InvoiceConference']['description']))); ?>
