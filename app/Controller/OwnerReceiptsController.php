...
class OwnerReceiptsController extends AppController {
    ...
    public function add() {
        ...
        $clients = $this->Receipt->Client->find('list', array('order' => 'Client.name', 'conditions' => array('Client.id' => $this->getPhkRequestVar('owner_id'))));
    ...
    public function pay_receipt($id = null) {
        ...
        $clients = $this->Receipt->Client->find('list', array('order' => 'Client.name', 'conditions' => array('id' => $this->getPhkRequestVar('owner_id'))));
    ...
    public function beforeRender() {
        ...
        switch ($this->action) {
            case 'add_notes':
                $breadcrumbs[7] = array('link' => Router::url(array('controller' => 'owner_receipts', 'action' => 'index', '?' => array('owner_id' => $this->getPhkRequestVar('owner_id'), 'fraction_id' => $this->getPhkRequestVar('fraction_id')))), 'text' => __n('Receipt', 'Receipts', 2), 'active' => '');
                $breadcrumbs[8] = array('link' => Router::url(array('controller' => 'owner_receipts', 'action' => 'view', $this->getPhkRequestVar('receipt_id'))), 'text' => $this->getPhkRequestVar('receipt_text'), 'active' => '');
